...
import type { GoogleResponse, AnthropicResponse, AnthropicContentBlock } from "./types.js";

/**
 * Convert Google Generative AI response to Anthropic Messages API format
 *
 * @param googleResponse - Google format response (the inner response object)
 * @param model - The model name used
 * @returns Anthropic format response
 */
export function convertGoogleToAnthropic(googleResponse: GoogleResponse, model: string): AnthropicResponse {
  ...
  for (const part of parts) {
    if ("text" in part && part.text !== undefined) {
      ...
      const toolUseBlock: AnthropicContentBlock & { thoughtSignature?: string } = {
        type: "tool_use",
        id: toolId,
